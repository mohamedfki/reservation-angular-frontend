...
export class ReservationsComponent implements OnInit {
  ...
  rservedReservation(id: number): void {
    this.http.get(`http://localhost:8080/api/reservations/reserved/${id}`).subscribe({
      next: (response) => {
...
addReservation() {
  ...
  saveUpdatedReservation(): void {
    this.http.put(`http://localhost:8080/api/reservations/${this.editingReservation.id}`, this.editingReservation).subscribe(
      () => {
